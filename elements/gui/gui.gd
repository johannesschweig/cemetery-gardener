...
func process_interaction(name: String):
	match name:
		"fountain":
			if inventory.get_status("hammer") == Utils.ItemStatus.INITIAL:
			...
			elif inventory.get_status("hammer") == Utils.ItemStatus.FOUND:
				return {
					"used": "hammer",
					"found": "mobile_phone",
		...
		"sofa": 
			if inventory.get_status("travel_flyer") == Utils.ItemStatus.INITIAL:
				return {
					"found": "travel_flyer",
		...
		"photo": 
			if inventory.get_status("letters") == Utils.ItemStatus.INITIAL:
				return {
					"found": "letters",
		...
		"wardrobe":
			if inventory.get_status("notes_with_code") == Utils.ItemStatus.INITIAL:
				return {
		...
		"leaning_spade":
			if inventory.get_status("spade") == Utils.ItemStatus.INITIAL:
				return {
					"found": "spade",
		...
		"flower_vases":
			if (inventory.get_status("spare_key") == Utils.ItemStatus.INITIAL):
				return {
					"found": "spare_key",
		...
		"carts":
			if inventory.get_status("car_key") == Utils.ItemStatus.INITIAL:
				return {
					"found": "car_key",
					"discovered": "opel",
		...
		"box":
			if inventory.get_status("newspaper_article") == Utils.ItemStatus.INITIAL:
				return {
					"found": "newspaper_article",
...
func click_poi(name: String):
	...
	if feedback is Dictionary:
		if feedback.has("found"):
			inventory.set_status(feedback.found, Utils.ItemStatus.FOUND)
		if feedback.has("used"):
			inventory.set_status(feedback.used, Utils.ItemStatus.USED)
