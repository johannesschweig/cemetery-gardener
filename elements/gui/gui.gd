...
func process_interaction(name: String):
	match name:
		"fountain":
			if inventory.get_status(24) == ItemStatus.INITIAL:
			...
			elif inventory.get_status(24) == ItemStatus.FOUND:
				inventory.set_status(25, ItemStatus.FOUND)
				inventory.set_status(24, ItemStatus.USED)
		...
		"police_car":
			if inventory.get_status(24) == ItemStatus.INITIAL:
				inventory.set_status(24, ItemStatus.FOUND)
		...
		"sofa": 
			if inventory.get_status(30) == ItemStatus.INITIAL:
				inventory.set_status(30, ItemStatus.FOUND)
				return "An old light brown leather sofa with a travel brochure lying on it."
			else:
				return "An old light brown leather sofa."
		"photo": 
			if inventory.get_status(34) == ItemStatus.INITIAL:
				inventory.set_status(34, ItemStatus.FOUND)
		...
		"wardrobe":
			if inventory.get_status(33) == ItemStatus.INITIAL:
				inventory.set_status(33, ItemStatus.FOUND)
		...
		"leaning_spade":
			if inventory.get_status(31) == ItemStatus.INITIAL:
				inventory.set_status(31, ItemStatus.FOUND)
				return "The spade still looks to be in good shape. You take it with you as a precaution."
			else:
				return "You picked up the spade here."
		"flower_vases":
			if (inventory.get_status(22) == ItemStatus.INITIAL):
				inventory.set_status(22, ItemStatus.FOUND)
		...
		"gate":
			if inventory.get_status(22) == ItemStatus.INITIAL:
				...
				inventory.set_status(22, ItemStatus.USED)
				return "The heavy metal key fits perfectly into the gate lock. You turn the key in the lock and open the gate."
		"carts":
			if inventory.get_status(20) == ItemStatus.INITIAL:
				inventory.set_status(20, ItemStatus.FOUND)
				# show_poi("opel") # TODO
		...
		"chest":
			inventory.set_status(35, ItemStatus.FOUND)
		...
		"box":
			if inventory.get_status(35) == ItemStatus.INITIAL:
				inventory.set_status(35, ItemStatus.FOUND)
...
func click_poi(name: String):
	var text = process_interaction(name)
	print(text)
